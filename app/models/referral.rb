class Referral < ApplicationRecord
  ...
  scope :active_statuses, -> { where.not(status: 'canceled') }
  scope :active_determination, -> { where.not(determination: 'accepted') }
  scope :active, -> { active_statuses.active_determination }
end
